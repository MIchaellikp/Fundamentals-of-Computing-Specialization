...
def draw(canvas):
    ...
    my_ship.update()
    # detect collision and update lives
...
def click(pos):
    ...
    if (not started) and width_splash and height_splash:
        started = True
        score = 0
        lives = 3
        timer.start()
...
def group_collide(group, other):
    ...
    for explosion in set(group):
        if explosion.collide(other):
            explosion_group.add(
                Sprite(explosion.get_position(), (0, 0), 0, 0, explosion_image, explosion_info, explosion_sound))
...
def group_group_collide(group, others_group):
    total = 0
    for collide in set(group):
        count = group_collide(others_group, collide)
        if count > 0:
